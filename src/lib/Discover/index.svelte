...
export let imageOverlay;

// learn: svelte.dev/repl/2d8c50325a614e0988761474f8e863aa?version=3.46.4
let activeIndex = 0;
let container;
...
function scrollActiveIntoView(index) {
	if (container) {
		container.children[index].scrollIntoView({ behavior: 'smooth', block: "end", inline: "nearest" });
	}
}
...
<Defs /><!-- debug: please move into `__layout`? -->


<aside class="bg-gold flex items-center x-mandatory overflow-x-auto overflow-y-hidden touch-scroll backface-hidden w-100 tc" use:trackIntersect bind:this={container}>

{#each items as {id, title, src, headingEn, headingIt, figCaption, slug}, i}

	<!-- debug: xxx -->
	<!-- fix: the `<Anchor />` -->
	<figure
		id={id}
		class="snap-start flex-column flex flex-shrink-0 grid white cover ma0"
		style="
		--alpha:0.6;
		min-width: 100%;
...
<div class="db black-70 f5 f4-ns f3-m f3-l lh-copy measure ph2 measure-ns ph4-ns measure-m ph2-m measure-wide-l ph0-l mr-auto ml-auto">
	...
	<nav class="fl w-100 cf flex justify-center">
		{#each items as {title}, i}
			<button
				style="width:calc(100% / {items.length})"
				class="o-50 f7 inline-flex items-center justify-center pv1 bg-transparent bn golden-brown"
